...
%% 采用交叉验证选择参数（需求二）
[bestacc,bestc,bestg] = SVMcg(output_train,input_train,-8,8,-8,8,3,0.5,0.5);

%% 使用选取的参数训练模型（需求三）
% 训练模型
options = ['-c ',num2str(bestc),' -g ',num2str(bestg),' -s 3 -p 0.01 -n 0.1 -t 1'];
...
% 预测结果绘图
figure;
plot(1:length(y_pred(:,3)),x_pred(:,3),'-*','linewidth',1.1,'color',[161/225 47/225 47/225]);  % 真实值
set(gca,'linewidth',1.4)
hold on;
plot(1:length(y_pred(:,3)),py_pred_guiyi,'-.o','linewidth',1.1,'color',[56/225 13/225 49/225]);   % 预测值
